#!/bin/bash
#
# ffmpeg video slideshow script with circular collapse transition v1 (23.01.2019)
#
...
[stream2starting]geq=lum='p(X,Y)':a='if(lte(pow(sqrt(pow(W/2,2)+pow(H/2,2))-sqrt(pow(T/1*W/2,2)+pow(T/1*H/2,2)),2),pow(X-(W/2),2)+pow(Y-(H/2),2)),255,0)'[stream2circularstarting];
[stream3starting]geq=lum='p(X,Y)':a='if(lte(pow(sqrt(pow(W/2,2)+pow(H/2,2))-sqrt(pow(T/1*W/2,2)+pow(T/1*H/2,2)),2),pow(X-(W/2),2)+pow(Y-(H/2),2)),255,0)'[stream3circularstarting];
[stream4starting]geq=lum='p(X,Y)':a='if(lte(pow(sqrt(pow(W/2,2)+pow(H/2,2))-sqrt(pow(T/1*W/2,2)+pow(T/1*H/2,2)),2),pow(X-(W/2),2)+pow(Y-(H/2),2)),255,0)'[stream4circularstarting];
[stream5starting]geq=lum='p(X,Y)':a='if(lte(pow(sqrt(pow(W/2,2)+pow(H/2,2))-sqrt(pow(T/1*W/2,2)+pow(T/1*H/2,2)),2),pow(X-(W/2),2)+pow(Y-(H/2),2)),255,0)'[stream5circularstarting];
[stream1ending][stream2circularstarting]overlay=0:0:shortest=1[stream2blended];\
[stream2ending][stream3circularstarting]overlay=0:0:shortest=1[stream3blended];\
[stream3ending][stream4circularstarting]overlay=0:0:shortest=1[stream4blended];\
[stream4ending][stream5circularstarting]overlay=0:0:shortest=1[stream5blended];\
[stream1overlaid][stream2blended][stream2overlaid][stream3blended][stream3overlaid][stream4blended][stream4overlaid][stream5blended][stream5overlaid]concat=n=9:v=1:a=0,format=yuv420p[video]"\
 -map [video] -vsync 2 -async 1 -rc-lookahead 0 -g 0 -profile:v main -level 42 -c:v libx264 -r 30 ../transition_collapse_circular.mp4
