#!/bin/bash
#
# ffmpeg video slideshow script with vertical cover transition v4 (10.12.2018)
#
...
do
    case ${DIRECTION} in
        1)
            FULL_SCRIPT+="[stream$((c+1))starting][stream${c}ending]blend=all_expr='if(gte(Y,H*T/${TRANSITION_DURATION}),B,A)':shortest=1[stream$((c+1))blended];"
        ;;
        *)
            FULL_SCRIPT+="[stream$((c+1))starting][stream${c}ending]blend=all_expr='if(gte(Y,H - H*T/${TRANSITION_DURATION}),A,B)':shortest=1[stream$((c+1))blended];"
        ;;
    esac
done
...
# 9. END
FULL_SCRIPT+=" -map [video] -vsync 2 -async 1 -rc-lookahead 0 -g 0 -profile:v main -level 42 -c:v libx264 -r ${FPS} ../transition_cover_vertical.mp4"
