#!/bin/bash
#
# ffmpeg video slideshow script with horizontal wipe in transition v5 (25.05.2019)
#
...
do
    case ${DIRECTION} in
        1)
            FULL_SCRIPT+="[stream${c}ending][stream$((c+1))starting]overlay=x='-w+t/${TRANSITION_DURATION}*${WIDTH}':y=0,select=lte(n\,${TRANSITION_FRAME_COUNT})[stream$((c+1))blended];"
        ;;
        *)
            FULL_SCRIPT+="[stream${c}ending][stream$((c+1))starting]overlay=x='w-t/${TRANSITION_DURATION}*${WIDTH}':y=0,select=lte(n\,${TRANSITION_FRAME_COUNT})[stream$((c+1))blended];"
        ;;
...
# 9. END
FULL_SCRIPT+=" -map [video] -vsync 2 -async 1 -rc-lookahead 0 -g 0 -profile:v main -level 42 -c:v libx264 -r ${FPS} ../transition_wipe_in_horizontal.mp4"
