#!/bin/bash
#
# ffmpeg video slideshow script with horizontal wipe in transition v1 (01.10.2017)
#
...
[stream1ending][stream2starting]overlay=x='-w+t/1*1280':y=0,select=lte(n\,30)[stream2blended];\
[stream2ending][stream3starting]overlay=x='-w+t/1*1280':y=0,select=lte(n\,30)[stream3blended];\
[stream3ending][stream4starting]overlay=x='-w+t/1*1280':y=0,select=lte(n\,30)[stream4blended];\
[stream4ending][stream5starting]overlay=x='-w+t/1*1280':y=0,select=lte(n\,30)[stream5blended];\
[stream1overlaid][stream2blended][stream2overlaid][stream3blended][stream3overlaid][stream4blended][stream4overlaid][stream5blended][stream5overlaid]concat=n=9:v=1:a=0,format=yuv420p[video]"\
 -map [video] -vsync 2 -async 1 -rc-lookahead 0 -g 0 -profile:v main -level 42 -c:v libx264 -r 30 ../transition_wipe_in_horizontal.mp4
