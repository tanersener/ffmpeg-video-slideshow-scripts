#!/bin/bash
#
# ffmpeg video slideshow script with vertical collapse transition v2 (04.12.2018)
#
...
for (( c=1; c<${PHOTOS_COUNT}; c++ ))
do
    FULL_SCRIPT+="[stream$((c+1))starting][stream${c}ending]blend=all_expr='if(gte(Y,(H/2)*T/${TRANSITION_DURATION})*lte(Y,H-(H/2)*T/${TRANSITION_DURATION}),B,A)':shortest=1[stream$((c+1))blended];"
done
...
# 9. END
FULL_SCRIPT+=" -map [video] -vsync 2 -async 1 -rc-lookahead 0 -g 0 -profile:v main -level 42 -c:v libx264 -r ${FPS} ../transition_collapse_vertical.mp4"
