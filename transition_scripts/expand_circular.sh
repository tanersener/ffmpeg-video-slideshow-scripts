...
for (( c=1; c<${PHOTOS_COUNT}; c++ ))
do
    FULL_SCRIPT+="[stream$((c+1))starting]geq=lum='p(X,Y)':a='if(lte(pow(sqrt(pow(T/${TRANSITION_DURATION}*W/2,2)+pow(T/${TRANSITION_DURATION}*H/2,2)),2),pow(X-(W/2),2)+pow(Y-(H/2),2)),0,255)'[stream$((c+1))circularstarting];"
...
# 9. END
FULL_SCRIPT+=" -map [video] -vsync 2 -async 1 -rc-lookahead 0 -g 0 -profile:v main -level 42 -c:v libx264 -r ${FPS} ../transition_expand_circular.mp4"
