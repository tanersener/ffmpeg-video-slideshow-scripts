#!/bin/bash
#
# ffmpeg video slideshow script with circular expand transition v1 (23.01.2019)
#
...
[stream2starting]geq=lum='p(X,Y)':a='if(lte(pow(sqrt(pow(T/1*W/2,2)+pow(T/1*H/2,2)),2),pow(X-(W/2),2)+pow(Y-(H/2),2)),0,255)'[stream2circularstarting];
[stream3starting]geq=lum='p(X,Y)':a='if(lte(pow(sqrt(pow(T/1*W/2,2)+pow(T/1*H/2,2)),2),pow(X-(W/2),2)+pow(Y-(H/2),2)),0,255)'[stream3circularstarting];
[stream4starting]geq=lum='p(X,Y)':a='if(lte(pow(sqrt(pow(T/1*W/2,2)+pow(T/1*H/2,2)),2),pow(X-(W/2),2)+pow(Y-(H/2),2)),0,255)'[stream4circularstarting];
[stream5starting]geq=lum='p(X,Y)':a='if(lte(pow(sqrt(pow(T/1*W/2,2)+pow(T/1*H/2,2)),2),pow(X-(W/2),2)+pow(Y-(H/2),2)),0,255)'[stream5circularstarting];
...
[stream1overlaid][stream2blended][stream2overlaid][stream3blended][stream3overlaid][stream4blended][stream4overlaid][stream5blended][stream5overlaid]concat=n=9:v=1:a=0,format=yuv420p[video]"\
 -map [video] -vsync 2 -async 1 -rc-lookahead 0 -g 0 -profile:v main -level 42 -c:v libx264 -r 30 ../transition_expand_circular.mp4
