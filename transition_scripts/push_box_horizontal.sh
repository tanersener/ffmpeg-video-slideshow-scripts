#!/bin/bash
#
# ffmpeg video slideshow script with horizontal push box transition v1 (28.06.2018)
#
...
[5:v][stream1prephasein]overlay=x='-w+t/0.5*1280':y=0,trim=duration=0.5,select=lte(n\,15)[stream1phasein];\
[5:v][stream1prephaseout]overlay=x='t/0.5*1280':y=0,trim=duration=0.5,select=lte(n\,15)[stream1phaseout];\
[stream1phaseout][stream2prephasein]overlay=x='-w+t/0.5*1280':y=0,trim=duration=0.5,select=lte(n\,15)[stream2phasein];\
[5:v][stream2prephaseout]overlay=x='t/0.5*1280':y=0,trim=duration=0.5,select=lte(n\,15)[stream2phaseout];\
[stream2phaseout][stream3prephasein]overlay=x='-w+t/0.5*1280':y=0,trim=duration=0.5,select=lte(n\,15)[stream3phasein];\
[5:v][stream3prephaseout]overlay=x='t/0.5*1280':y=0,trim=duration=0.5,select=lte(n\,15)[stream3phaseout];\
[stream3phaseout][stream4prephasein]overlay=x='-w+t/0.5*1280':y=0,trim=duration=0.5,select=lte(n\,15)[stream4phasein];\
[5:v][stream4prephaseout]overlay=x='t/0.5*1280':y=0,trim=duration=0.5,select=lte(n\,15)[stream4phaseout];\
[stream4phaseout][stream5prephasein]overlay=x='-w+t/0.5*1280':y=0,trim=duration=0.5,select=lte(n\,15)[stream5phasein];\
[5:v][stream5prephaseout]overlay=x='t/0.5*1280':y=0,trim=duration=0.5,select=lte(n\,15)[stream5phaseout];\
...
concat=n=31:v=1:a=0,format=yuv420p[video]"\
 -map [video] -vsync 2 -async 1 -rc-lookahead 0 -g 0 -profile:v main -level 42 -c:v libx264 -r 30 ../transition_push_box_horizontal.mp4
