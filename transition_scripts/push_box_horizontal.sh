#!/bin/bash
#
# ffmpeg video slideshow script with horizontal push box transition v4 (25.05.2019)
#
...
do
    case ${DIRECTION} in
        1)
            FULL_SCRIPT+="[${IMAGE_COUNT}:v][stream${c}prephaseout]overlay=x='t/(${TRANSITION_DURATION}/2)*${WIDTH}':y=0,trim=duration=${TRANSITION_PHASE_DURATION},select=lte(n\,(${TRANSITION_FRAME_COUNT}/2))[stream${c}phaseout];"
