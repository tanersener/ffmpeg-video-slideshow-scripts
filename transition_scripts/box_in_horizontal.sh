...
do
    case ${DIRECTION} in
        1)
            FULL_SCRIPT+="[${IMAGE_COUNT}:v][stream${c}prephasein]overlay=x='-${WIDTH}+${WIDTH}*t/(${TRANSITION_DURATION}/2)':y=0,trim=duration=${TRANSITION_PHASE_DURATION},select=lte(n\,(${TRANSITION_FRAME_COUNT}/2))[stream${c}phasein];"
            FULL_SCRIPT+="[${IMAGE_COUNT}:v][stream${c}prephaseout]overlay=x='-${WIDTH}*t/(${TRANSITION_DURATION}/2)':y=0,trim=duration=${TRANSITION_PHASE_DURATION},select=lte(n\,(${TRANSITION_FRAME_COUNT}/2))[stream${c}phaseout];"
        ;;
        *)
            FULL_SCRIPT+="[${IMAGE_COUNT}:v][stream${c}prephasein]overlay=x='${WIDTH}-${WIDTH}*t/(${TRANSITION_DURATION}/2)':y=0,trim=duration=${TRANSITION_PHASE_DURATION},select=lte(n\,(${TRANSITION_FRAME_COUNT}/2))[stream${c}phasein];"
            FULL_SCRIPT+="[${IMAGE_COUNT}:v][stream${c}prephaseout]overlay=x='${WIDTH}*t/(${TRANSITION_DURATION}/2)':y=0,trim=duration=${TRANSITION_PHASE_DURATION},select=lte(n\,(${TRANSITION_FRAME_COUNT}/2))[stream${c}phaseout];"
