#!/bin/bash
#
# ffmpeg video slideshow script with horizontal box in transition v1 (28.06.2018)
#
...
[5:v][stream1prephasein]overlay=x='-1280 + 1280*t/0.5':y=0,trim=duration=0.5,select=lte(n\,15)[stream1phasein];\
[5:v][stream2prephasein]overlay=x='-1280 + 1280*t/0.5':y=0,trim=duration=0.5,select=lte(n\,15)[stream2phasein];\
[5:v][stream3prephasein]overlay=x='-1280 + 1280*t/0.5':y=0,trim=duration=0.5,select=lte(n\,15)[stream3phasein];\
[5:v][stream4prephasein]overlay=x='-1280 + 1280*t/0.5':y=0,trim=duration=0.5,select=lte(n\,15)[stream4phasein];\
[5:v][stream5prephasein]overlay=x='-1280 + 1280*t/0.5':y=0,trim=duration=0.5,select=lte(n\,15)[stream5phasein];\
[5:v][stream1prephaseout]overlay=x='-1280*t/0.5':y=0,trim=duration=0.5,select=lte(n\,15)[stream1phaseout];\
[5:v][stream2prephaseout]overlay=x='-1280*t/0.5':y=0,trim=duration=0.5,select=lte(n\,15)[stream2phaseout];\
[5:v][stream3prephaseout]overlay=x='-1280*t/0.5':y=0,trim=duration=0.5,select=lte(n\,15)[stream3phaseout];\
[5:v][stream4prephaseout]overlay=x='-1280*t/0.5':y=0,trim=duration=0.5,select=lte(n\,15)[stream4phaseout];\
[5:v][stream5prephaseout]overlay=x='-1280*t/0.5':y=0,trim=duration=0.5,select=lte(n\,15)[stream5phaseout];\
...
[stream1prezoomin]zoompan=z='min(pzoom+0.04,2)':d=1:fps=30:x='trunc(iw/2)-trunc(trunc(iw/zoom)/2)':y='oh/2-(ih/zoom/2)':s=1280x720,setpts=0.5*PTS[stream1zoomin];\
[stream2prezoomin]zoompan=z='min(pzoom+0.04,2)':d=1:fps=30:x='trunc(iw/2)-trunc(trunc(iw/zoom)/2)':y='oh/2-(ih/zoom/2)':s=1280x720,setpts=0.5*PTS[stream2zoomin];\
[stream3prezoomin]zoompan=z='min(pzoom+0.04,2)':d=1:fps=30:x='trunc(iw/2)-trunc(trunc(iw/zoom)/2)':y='oh/2-(ih/zoom/2)':s=1280x720,setpts=0.5*PTS[stream3zoomin];\
[stream4prezoomin]zoompan=z='min(pzoom+0.04,2)':d=1:fps=30:x='trunc(iw/2)-trunc(trunc(iw/zoom)/2)':y='oh/2-(ih/zoom/2)':s=1280x720,setpts=0.5*PTS[stream4zoomin];\
[stream5prezoomin]zoompan=z='min(pzoom+0.04,2)':d=1:fps=30:x='trunc(iw/2)-trunc(trunc(iw/zoom)/2)':y='oh/2-(ih/zoom/2)':s=1280x720,setpts=0.5*PTS[stream5zoomin];\
[stream1prezoomout]zoompan=z='2-in*0.04':d=1:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s=1280x720,setpts=0.5*PTS[stream1zoomout];\
[stream2prezoomout]zoompan=z='2-in*0.04':d=1:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s=1280x720,setpts=0.5*PTS[stream2zoomout];\
[stream3prezoomout]zoompan=z='2-in*0.04':d=1:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s=1280x720,setpts=0.5*PTS[stream3zoomout];\
[stream4prezoomout]zoompan=z='2-in*0.04':d=1:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s=1280x720,setpts=0.5*PTS[stream4zoomout];\
[stream5prezoomout]zoompan=z='2-in*0.04':d=1:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s=1280x720,setpts=0.5*PTS[stream5zoomout];\
[stream1phasein][stream1checkin][stream1zoomin][stream1overlaid][stream1zoomout][stream1checkout][stream1phaseout]\
[stream2phasein][stream2checkin][stream2zoomin][stream2overlaid][stream2zoomout][stream2checkout][stream2phaseout]\
[stream3phasein][stream3checkin][stream3zoomin][stream3overlaid][stream3zoomout][stream3checkout][stream3phaseout]\
[stream4phasein][stream4checkin][stream4zoomin][stream4overlaid][stream4zoomout][stream4checkout][stream4phaseout]\
[stream5phasein][stream5checkin][stream5zoomin][stream5overlaid][stream5zoomout][stream5checkout][stream5phaseout]\
concat=n=35:v=1:a=0,format=yuv420p[video]"\
 -map [video] -vsync 2 -async 1 -rc-lookahead 0 -g 0 -profile:v main -level 42 -c:v libx264 -r 30 ../transition_box_in_horizontal.mp4
