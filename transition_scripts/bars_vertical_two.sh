#!/bin/bash
#
# ffmpeg video slideshow script with vertical bars #2 transition v4 (25.05.2019)
#
...
BAR_COUNT=16                # WIDTH SHOULD BE DIVISIBLE BY BAR_COUNT. IF NOT VERTICAL LINES MAY APPEAR ON TRANSITION
...
for (( c=1; c<${PHOTOS_COUNT}; c++ ))
do
    FULL_SCRIPT+="[stream$((c+1))starting][stream${c}ending]blend=all_expr='if((lte(mod(X,(${WIDTH}/${BAR_COUNT})),(${WIDTH}/${BAR_COUNT})/2-((${WIDTH}/${BAR_COUNT})/2)*T/${TRANSITION_DURATION}))+(gte(mod(X,(${WIDTH}/${BAR_COUNT})),((${WIDTH}/${BAR_COUNT})/2)+((${WIDTH}/${BAR_COUNT})/2)*T/${TRANSITION_DURATION})),B,A)':shortest=1[stream$((c+1))blended];"
done
...
# 9. END
FULL_SCRIPT+=" -map [video] -vsync 2 -async 1 -rc-lookahead 0 -g 0 -profile:v main -level 42 -c:v libx264 -r ${FPS} ../transition_bars_vertical_two.mp4"
