#!/bin/bash
#
# ffmpeg video slideshow script with horizontal & vertical collapse transition v1 (01.10.2017)
#
...
[stream2starting][stream1ending]blend=all_expr='if((gte(X,(W/2)*T/1)*gte(Y,(H/2)*T/1))*(lte(X,W-(W/2)*T/1)*lte(Y,H-(H/2)*T/1)),B,A)':shortest=1[stream2blended];\
[stream3starting][stream2ending]blend=all_expr='if((gte(X,(W/2)*T/1)*gte(Y,(H/2)*T/1))*(lte(X,W-(W/2)*T/1)*lte(Y,H-(H/2)*T/1)),B,A)':shortest=1[stream3blended];\
[stream4starting][stream3ending]blend=all_expr='if((gte(X,(W/2)*T/1)*gte(Y,(H/2)*T/1))*(lte(X,W-(W/2)*T/1)*lte(Y,H-(H/2)*T/1)),B,A)':shortest=1[stream4blended];\
[stream5starting][stream4ending]blend=all_expr='if((gte(X,(W/2)*T/1)*gte(Y,(H/2)*T/1))*(lte(X,W-(W/2)*T/1)*lte(Y,H-(H/2)*T/1)),B,A)':shortest=1[stream5blended];\
[stream1overlaid][stream2blended][stream2overlaid][stream3blended][stream3overlaid][stream4blended][stream4overlaid][stream5blended][stream5overlaid]concat=n=9:v=1:a=0,format=yuv420p[video]"\
 -map [video] -vsync 2 -async 1 -rc-lookahead 0 -g 0 -profile:v main -level 42 -c:v libx264 -r 30 ../transition_collapse_both.mp4
