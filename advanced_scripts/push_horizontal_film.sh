#!/bin/bash
#
# ffmpeg video slideshow script with advanced push horizontal film v1 (01.10.2017)
#
...
-loop 1 -i ../objects/film_strip.png \
...
[6:v][stream1ending]overlay=x='t/2*1280':y=0,trim=duration=2,select=lte(n\,60)[stream1moving];\
[6:v][stream2ending]overlay=x='t/2*1280':y=0,trim=duration=2,select=lte(n\,60)[stream2moving];\
[6:v][stream3ending]overlay=x='t/2*1280':y=0,trim=duration=2,select=lte(n\,60)[stream3moving];\
[6:v][stream4ending]overlay=x='t/2*1280':y=0,trim=duration=2,select=lte(n\,60)[stream4moving];\
[6:v][stream5ending]overlay=x='t/2*1280':y=0,trim=duration=2,select=lte(n\,60)[stream5moving];\
[6:v][stream1starting]overlay=x='-w+t/2*1280':y=0:shortest=1,trim=duration=2,select=lte(n\,60)[stream1blended];\
[stream1moving][stream2starting]overlay=x='-w+t/2*1280':y=0:shortest=1,trim=duration=2,select=lte(n\,60)[stream2blended];\
[stream2moving][stream3starting]overlay=x='-w+t/2*1280':y=0:shortest=1,trim=duration=2,select=lte(n\,60)[stream3blended];\
[stream3moving][stream4starting]overlay=x='-w+t/2*1280':y=0:shortest=1,trim=duration=2,select=lte(n\,60)[stream4blended];\
[stream4moving][stream5starting]overlay=x='-w+t/2*1280':y=0:shortest=1,trim=duration=2,select=lte(n\,60)[stream5blended];\
[stream1blended][stream2blended][stream3blended][stream4blended][stream5blended][stream5moving]concat=n=6:v=1:a=0,format=yuv420p[video]"\
 -map [video] -vsync 2 -async 1 -rc-lookahead 0 -g 0 -profile:v main -level 42 -c:v libx264 -r 30 ../advanced_push_horizontal_film.mp4
