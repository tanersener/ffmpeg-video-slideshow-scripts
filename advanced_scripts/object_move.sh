#!/bin/bash
#
# ffmpeg video slideshow script with advanced object move v1 (01.10.2017)
#
...
[stream1overlaid][stream2blended][stream2overlaid][stream3blended][stream3overlaid][stream4blended][stream4overlaid][stream5blended][stream5overlaid]concat=n=9:v=1:a=0,format=yuv420p[videowithoutobject];\
[5:v]setpts=PTS-STARTPTS,setsar=sar=1/1,trim=duration=15,select=lte(n\,450),split=5[flake1][flake2][flake3][flake4][flake5];\
[videowithoutobject][flake1]overlay=y='mod(t/2*(1280/3),main_h + 16*overlay_h)-14*overlay_h':x=650:format=rgb,trim=duration=15,select=lte(n\,450)[videowith1flake];\
[videowith1flake][flake2]overlay=y='mod(t/2*(1280/3),main_h + 2*overlay_h)-overlay_h':x=1050:format=rgb,trim=duration=15,select=lte(n\,450)[videowith2flakes];\
[videowith2flakes][flake3]overlay=y='mod(t/2*(1280/3),main_h + 8*overlay_h)-6*overlay_h':x=400:format=rgb,trim=duration=15,select=lte(n\,450)[videowith3flakes];\
[videowith3flakes][flake4]overlay=y='mod(t/2*(1280/3),main_h + 6*overlay_h)-4*overlay_h':x=800:format=rgb,trim=duration=15,select=lte(n\,450)[videowith4flakes];\
[videowith4flakes][flake5]overlay=y='mod(t/2*(1280/3),main_h + 12*overlay_h)-10*overlay_h':x=200:format=rgb,trim=duration=15,select=lte(n\,450)[video]"\
 -map [video] -vsync 2 -async 1 -rc-lookahead 0 -g 0 -profile:v main -level 42 -pix_fmt yuv420p -c:v libx264 -r 30 ../advanced_object_move.mp4
