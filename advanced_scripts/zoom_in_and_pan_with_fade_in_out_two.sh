#!/bin/bash
#
# ffmpeg video slideshow script with zoom in and pan and fade in/out #2 transition v2 (20.05.2019)
#
...
do
    ...
    case ${PHOTO_MODE} in
        1)
            ...
            FULL_SCRIPT+="[stream$((c+1))out2]pad=width=${WIDTH}:height=${HEIGHT}:x=(${WIDTH}-iw)/2:y=(${HEIGHT}-ih)/2:color=${BACKGROUND_COLOR},scale=${WIDTH}*5:-1,zoompan=z='pzoom+0.0017*${TRANSITION_FRAME_COUNT}':d=max(${PHOTO_FRAME_COUNT}\,${TRANSITION_FRAME_COUNT}):${POSITION_FORMULA}:fps=${FPS}:s=${WIDTH}x${HEIGHT},split=2[stream$((c+1))outzoom1][stream$((c+1))outzoom2];"
        ;;
        *)
